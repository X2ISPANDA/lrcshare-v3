...
function buildSongData(row: BatchRow) {
  ...
  return {
    ...
    album_artist: albumArtists.map(a => a.name).join(' / '),
    lyricist: lyricists.map(a => a.name).join(' / '),
    composer: composers.map(a => a.name).join(' / '),
    arranger: arrangers.map(a => a.name).join(' / '),
    album: effAlbumName,
    album_id: effAlbumId,
