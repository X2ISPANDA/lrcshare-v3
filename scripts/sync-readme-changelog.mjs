 ...
 * 输出：CHANGELOG.md（项目根，独立于 README）
 *
 * 用法：
 *   node scripts/sync-readme-changelog.mjs                 默认：changelog.ts → CHANGELOG.md
 ...
 * - README 的「更新日志」章节已改为静态 stub（指向 CHANGELOG.md 与主站 /changelog），本脚本不再触碰 README
 */
...
/** changelog 数据 → CHANGELOG.md 内容 */
function renderChangelogMd(data) {
...
/** README「更新日志」章节 → changelog 数据（迁移/兜底解析，仅 --from-readme 用） */
...
function writeChangelogMd(data) {
  const content = renderChangelogMd(data)
  writeFileSync(CHANGELOG_MD, content, 'utf8')
}
 ...
 * - CHANGELOG.md 由 scripts/sync-readme-changelog.mjs 从本文件生成，勿手改 CHANGELOG.md
 * - README.md 的「更新日志」章节为静态 stub，指向 CHANGELOG.md 与主站 /changelog，本脚本不再触碰 README
...
if (fromReadme) {
  ...
  writeChangelogMd(data)
  const n = data.reduce((s, d) => s + d.entries.length, 0)
  console.log(`[changelog] src/data/changelog.ts → CHANGELOG.md：${data.length} 个日期 / ${n} 条`)
}
