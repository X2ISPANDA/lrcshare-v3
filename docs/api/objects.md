...
| `album` | `{id, name, year, cover, artists}` \| null | TALB | ALBUM | 所属专辑（单曲专辑也是专辑）；`cover` 为专辑封面，全站唯一封面来源；`artists` 为专辑艺术家（TPE2 / ALBUMARTIST） |
...
| `lrc` | string \| null | USLT | LYRICS | 完整时间轴 LRC 歌词，末尾自动追加一行来源署名（无时间戳）；带歌词参数时为按语言圈定后的合成结果，见[获取歌词](/api/lyric#get-lyric) |
| `lyrics` | `{lang, kind, format, lrc}[]` \| null | — | — | 按语言参数选中的版本数组，每项的 `lrc` 为该版本独立完整歌词文本（仅带 `lyric_lang` / `lyric_translation_lang` 请求时返回） |
| `lyric_lines` | object \| null | — | — | 结构化多语言歌词（`lyric_lines=1` 时返回）：`{primary_lang, versions[]}`，每个版本 `{lang, kind, rows[]}`，`kind` 为 `original` / `translation` / `romanization`，结构见[lyric_lines 结构化行](/api/lyric#lyric-lines) |
