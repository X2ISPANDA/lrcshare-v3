...
多艺人合作曲的查询支持两种传法，**推荐用数组**：

**数组模式（推荐）**：`artist` 参数重复传值，每个元素是一个完整艺人名。语义精确——元素不会被再拆分（名字本身含 `/` 的艺人如 `AC/DC` 作为整名匹配），元素之间为 AND 关系（合作曲的艺术家名单需同时命中每个元素）。适用于调用方已掌握结构化多艺人数据的场景（如音乐软件的用户曲库元数据）。

```bash
# 查 BuzzY.D 与 NKidd 的合作曲
curl "https://api.lrcshare.com/v1/search?type=song&artist=BuzzY.D&artist=NKidd"

# 名字含斜杠的艺人也按整名处理，不会被拆
curl "https://api.lrcshare.com/v1/search?type=song&artist=AC/DC&artist=Bob"
```

**斜杠字符串模式（兜底）**：单值 `artist=A/B/C`，服务端按 `/` 拆分后 AND 匹配。适用于调用方只有一个拼接串的场景（如 tag 的 `TPE1=A/B/C`）。规则与限制：

- **整串恰好等于库内某艺人名/别名时（如 `AC/DC`），视为一个艺人按整名匹配**，不再拆分——搜 `AC/DC` 返回的是该乐队的歌，而不是「AC 和 DC 的合作曲」
- 整串不是库内艺人名时按 `/` 拆段（每段去首尾空白，**单字符段丢弃**——防止单字母子串灌水），每段子串匹配艺术家名/别名，AND 语义
- 拆段匹配是**子串**语义：名字含 `/` 的艺人（如 `R/Aph`）无需拼回——查询 `R/Aph/Bob` 拆出的 `Aph`、`Bob` 段可直接命中其名字
- 极端场景不可解：查询整串恰为艺人名但你想按组合拆（歧义）、双单字符斜杠艺人（如 `X/Y` 拆段全被丢弃）。**遇到这类情况用 `title` 参数锁定**，标题匹配不受斜杠规则影响

数组模式仅 `type=song` 支持（`type=album` 传多个 `artist` 返回 400）。
